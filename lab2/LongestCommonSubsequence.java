...
public class LongestCommonSubsequence {
    ...
    public static void main(String args[]) {
        ...
        // algorithm
        while(possibleSubsequences.getCurrentSize() > 0) {
            ...
            if(test.length() > longest.length()) {
                if(isSubsequence(test, second)) {
                    ...
                    for (int i=0; i < test.length(); i++) {
                        possibleSubsequences.add(test.substring(0, i) + test.substring(i+1));
                    }
                }
            }
            System.out.println("Current Bag: " + possibleSubsequences.toString());
        }
