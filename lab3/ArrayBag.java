...
public class ArrayBag<E> implements BagInterface<E> {
    ...
    */
    ...
    public boolean equals(Object other) {
        ...
        if((other instanceof ArrayBag) == false) return false;
        if(size != ((ArrayBag)other).getCurrentSize()) return false;
        for(int i=0; i<size; i++) {
            if(((ArrayBag)other).contains(bag[i])) {
                if(this.getFrequencyOf(bag[i]) != ((ArrayBag) other).getFrequencyOf(bag[i])) {
                    return false;
                }
            }
        }
