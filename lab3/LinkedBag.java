...
public final class LinkedBag<E> implements BagInterface<E> {
    ...
    public boolean equals(Object other) {
        ...
        if((other instanceof LinkedBag) == false) return false;
        if(size != ((LinkedBag)other).getCurrentSize()) return false;
        Node scout = head;
        int i = 0;
        while(i<size) {
            if(((LinkedBag)other).getReferenceTo(scout.data) != null) {
                if(this.getFrequencyOf(scout.data) == ((LinkedBag)other).getFrequencyOf(scout.data)) {
                    scout = scout.next;
                    i++;
                } else {
                    return false;
